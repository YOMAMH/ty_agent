...
function mkLogEmitter(minLevel) {
    ...
    return function () {
        ...
        var str = curr_date_string() + ' (pid:' + this.ids.pid + user_id + ') ' + nameFromLevel[minLevel] + ' <' + name + '> ';
        fs.appendFile(this.options.path, str + this.format.apply(this, arguments)+'\n', function cb(){});
    }
}
...
function fix_level (value) {
    ...
    else if (LEVELS.indexOf(value) === -1) value = 'info';
    return value;
}
Logger.prototype.config = function(options) {
    var result = {
        level   : fix_level(options.agent_log_level),
        path    : options.agent_log_file_name,
        count   : ( (typeof options.max_log_count !== 'undefined') ? options.max_log_count: 3 ),
        size    : ( (typeof options.max_log_size !== 'undefined') ? options.max_log_size: 1024 * 1024 * 10),
        zip     : ( (typeof options.zip_log !== 'undefined') ? options.zip_log : true )
    };
    if (! result.path ) {
        result.path = path.join(__dirname, '../../../tingyun_agent.log');
    }
    return result;
};
